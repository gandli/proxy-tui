...
pub fn run(config: &Path, public_ip: &str) -> anyhow::Result<()> {
    reality_scan::download(config, &RealExecutor).map_err(|e| anyhow::anyhow!(e))?;
    println!("扫描器已就绪,开始扫描 {public_ip} ...");
    let domains =
        reality_scan::scan(public_ip, config, &RealExecutor).map_err(|e| anyhow::anyhow!(e))?;
    if domains.is_empty() {
